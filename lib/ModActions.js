	...
	constructor(bot)
	{
		...
		this.muteTimer = new Promise(() =>
		{
			setInterval(() =>
			{
				...
				let oldMutes = activeMutes;
				let newMutes = JSON.parse(JSON.stringify(oldMutes));
				Object.keys(oldMutes).forEach(key =>
				{
					let mute = oldMutes[key];
					if (Time.difference(mute.duration * 1000 * 60 * 60, Time.now() - mute.timestamp).ms < 1)
					{
						console.log(`Removing expired mute for user '${mute.user}'`);
